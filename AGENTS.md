...
- Regions own botanical intent only — positions, archetype ids, yaw, scale. The shared runtime owns compilation, instancing/batching, wind shading, LOD grades, and chunk culling. Do not duplicate any of those per region. Foliage never casts or receives shadows; do not add plant shadow proxies or static-shadow invalidations for foliage residency/visibility changes.
