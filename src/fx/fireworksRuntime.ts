...
export class Fireworks {
  ...
  async prepare(compile: (owner: THREE.Object3D) => Promise<void>): Promise<void> {
    this.#sprite.visible = true;
    this.#sprite.count = 1;
    try { await compile(this.#sprite); }
    finally { this.#sprite.visible = false; this.#sprite.count = 0; }
  }
  ...
  dispose(): void {
    ...
    for (const node of [this.#pPos, this.#pVel, this.#pLife, this.#pColA, this.#pColB, this.#cmdBuf]) releaseRendererAttribute(node.value);
    if (this.#audioWarmTimer !== null) window.clearTimeout(this.#audioWarmTimer);
  }
