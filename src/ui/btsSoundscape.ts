 ...
 * about the procedural nature-audio engine (src/audio), threaded with three
 * hand-drawn SVG diagrams. Two of them are *scroll-driven* — a sticky graphic
 * that advances through stages as you scroll its captions past it, then releases
 * and lets you read on. The third breathes on its own, a slow day passing.
 *
 * This module owns only the tab's inner HTML plus a small controller that maps
 * scroll position → diagram stage and runs one gentle rAF while the tab is open.
 * The panel (behindTheScenes.ts) mounts it and calls activate()/deactivate() as
 * you switch tabs, so nothing animates when nobody's looking.
 */
...
const DIAGRAM_ARCH = `
  <svg viewBox="0 0 820 320" class="ss-svg" role="img" aria-label="From regions to the audio bus">
    <g id="ss-map">
      <rect x="24" y="40" width="212" height="244" rx="20" fill="rgba(20,46,64,0.55)" stroke="rgba(127,224,205,0.35)"/>
    ...
    <g id="ss-beds" opacity="0">
      <rect x="300" y="60" width="252" height="56" rx="10" fill="rgba(18,40,56,0.6)" stroke="rgba(127,224,205,0.3)"/>
    ...
    <g id="ss-wind" opacity="0">
      <rect x="300" y="138" width="252" height="46" rx="10" fill="rgba(18,40,56,0.6)" stroke="rgba(127,224,205,0.3)"/>
    ...
    <g id="ss-voices" opacity="0">
      <rect x="300" y="206" width="252" height="56" rx="10" fill="rgba(18,40,56,0.6)" stroke="rgba(127,224,205,0.3)"/>
...
const DIAGRAM_SONG = `
  <svg viewBox="0 0 820 300" class="ss-svg" role="img" aria-label="Building one bird call">
    <g id="ss-graph">
      ...
      <path id="ss-wave" d="M60 150 L600 150" fill="none" stroke="#9ef2df" stroke-width="1.8"/>
      <g>
    ...
    <g id="ss-plan" opacity="0">
      ...
      <circle cx="410" cy="150" r="7" fill="rgba(127,224,205,0.9)"/>
      ...
      <circle id="ss-caller" cx="486" cy="120" r="5" fill="#9ef2df"/>
      <circle id="ss-answer" cx="338" cy="128" r="4.5" fill="#7fe0cd" opacity="0"/>
...
const DIAGRAM_DAY = `
  <svg viewBox="0 0 820 270" class="ss-svg" role="img" aria-label="Call density across a day">
    ...
    <circle id="ss-nowdot" cx="60" cy="200" r="4" fill="#eafff9"/>
    <circle id="ss-sun" cx="60" cy="150" r="10" fill="rgba(255,214,140,0.95)"/>
...
export const SOUNDSCAPE_TAB_HTML = `
  <section>
    ...
    time, the same way the city itself is built from data rather than modelled by hand. This is how it
    works — best read slowly, ideally while it's playing.</p>
  </section>
  ...
  <section>
    ...
    stealing frames from the renderer.</p>
  </section>
  ...
  <section>
    ...
    a real hillside goes quiet in a strong wind. Watch a full day go by.</p>
  ...
  <section>
    ...
    made of nothing but oscillators and noise, decided fresh each time. Sit with it a while. It's trying,
    in its small way, to be a place rather than a track.</p>
  </section>
`;
...
// Diagram 3 — a full day passing on its own: sun/moon arc, call-density curve,
// and the bird↔cricket crossfade underneath. Purely idle-animated.
function renderDay(svg: SVGSVGElement, t: number) {
  const hour = (t * (24 / 48)) % 24; // one full day every ~48s
...
export function dayCurvePath(): string {
  let d = "";
  for (let i = 0; i <= 96; i++) {
    const h = (i / 96) * 24;
    const x = 60 + (700 * h) / 24;
