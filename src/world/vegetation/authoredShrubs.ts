...
const PROFILES: Record<AuthoredShrubProfile, {
  ...
  squareness?: number;
  /** Distribute sprays through the crown volume instead of one radial shell. */
  filledVolume?: boolean;
  /** Blooms are small petal clusters, never whole leaves recolored pink. */
  petalBlooms?: boolean;
}> = {
...
/**
 * Five thin petals around a bright heart — reads as a real flower cluster at
 * gameplay distance instead of a whole leaf recolored pink.
 */
...
function makeLeafSprayGeometry(profile: AuthoredShrubProfile): THREE.BufferGeometry {
  ...
  for (let i = 0; i < p.count; i++) {
    ...
    // Filled profiles scatter sprays through the crown volume; shell profiles
    // keep the classic single-envelope silhouette.
    const shell = filled
    ...
    // Shell profiles drop every third leaf to an inner shell with a dimmer
    // shade — depth between the core hull and the silhouette. Filled profiles
    // already have interior sprays, so they skip the trick.
    const inner = !filled && profile !== "fern" && i % 3 === 2;
    ...
    pushLeaf(
      ...
      phase,
      !p.petalBlooms && !inner && i % p.bloomEvery === 0 && level > 0.42 ? 1 : 0,
      inner ? 0.78 : 1
    );
