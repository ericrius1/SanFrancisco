...
const habitat = (id: string, species: BirdSpeciesId, x: number, z: number, count: number, radius: number, seed: number): BirdHabitat => ({
  id, species, center: { x, y: 55, z }, count, radius, seed, loadDistance: 660, unloadDistance: 850,
});
const landmarks = [
...
for (let row = 0, z = -2300; z <= 4800; row++, z += 780) {
  for (let col = 0, x = -6000; x <= 4500; col++, x += 780) {
    const seed = 200 + row * 31 + col * 13;
    const px = x + ((seed * 37) % 181 - 90), pz = z + ((seed * 53) % 181 - 90);
    if (landmarks.some(h => Math.hypot(h.center.x - px, h.center.z - pz) < 360)) continue;
    ...
    // Single scouts, pairs, loose parties, and occasional neighborhood flocks.
    const count = [1, 2, 4, 7, 12][seed % 5];
