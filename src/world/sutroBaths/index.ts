  ...
  return {
    ...
    update(dt, time, player, camera, gust) {
      ...
      if (inGrotto) {
        ...
        // than anywhere, since nothing outside is observable at all, by the
        // player or by the camera (which is in the same sealed room).
        twilight.update(dt, player, camera.position, true);
    ...
    takeFloorHandoffHeight(player, playerMode) {
      ...
      const feetY = y === undefined ? null : y - WALK_CAPSULE_HALF_EXTENT;
