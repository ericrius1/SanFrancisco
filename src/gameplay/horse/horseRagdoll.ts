// Adapted from the horse-rl-gait-search branch: it imported box3d-wasm's wrapper
// directly, but main now runs box3d.js behind src/core. The in-browser ragdoll
// lives in its OWN private box3d world (a flat ground plane) built from main's
// Box3D facade, which exposes the exact create-body/joint/impulse surface the
// branch used — so the port is just this import + the world/box3d types.
...
export class HorseRagdoll {
  ...
  constructor(box3d: Box3D, spec: CreatureSpec, policyDef: PolicyDef, scale = 1) {
  ...
  jump(): void {
    ...
    const vUp = 0.82 * this.speedUnit; // flatter, softer arc = easier landing (still clears the low rails)
