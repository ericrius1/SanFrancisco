...
export function buildScooterMesh(raw?: ScooterConfig): THREE.Group {
  ...
  const mesh = (geometry: THREE.BufferGeometry, material: THREE.Material, x: number, y: number, z: number, parent: THREE.Object3D = root) => {
  ...
  const box = (material: THREE.Material, w: number, h: number, d: number, x: number, y: number, z: number, parent?: THREE.Object3D) =>
  ...
  for (const z of [-0.98, 0.98]) {
    ...
    mesh(tireGeo, rubber, 0, 0, 0, wheel);
  ...
  box(paint, 0.64, 0.22, 1.05, 0, 0.34, 0.32);
  const haunch = mesh(roundedBox(0.75, config.body === "sport" ? 0.66 : 0.76, 0.82, 0.18), paint, 0, 0.55, 0.63);
  haunch.rotation.x = -0.04;
  const shieldHeight = config.body === "touring" ? 1.16 : config.body === "sport" ? 0.88 : 1.03;
  const shield = mesh(roundedBox(config.body === "sport" ? 0.72 : 0.82, shieldHeight, 0.18, 0.2), paint, 0, 0.72, -0.67);
  ...
  const seat = mesh(roundedBox(0.58, 0.18, seatLength, 0.16), seatMat, 0, 1.02, seatZ);
  ...
  if (config.cargo === "topbox") {
    const topbox = mesh(roundedBox(0.66, 0.42, 0.5, 0.13), paint, 0, 1.27, 1.29);
