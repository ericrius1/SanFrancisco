...
// vendored from src/nodes/utils/PostProcessingUtils.js at the same commit; three 0.185
// does not export getScreenPositionFromClip yet
...
class SSAONode extends TempNode {
	...
	setup( builder ) {
		...
		const ao = Fn( () => {
			...
			// vendor tweak: no-write pixels (sky) clear to 1.0 with a standard depth
			// buffer and 0.0 with a reversed one — bail on both ends

			depth.greaterThanEqual( 1.0 ).or( depth.lessThanEqual( 0.0 ) ).discard();
		...
		// vendor tweak: only built when enabled, so the disabled path can't trip on it
