...
/** Pool placeholders have no vertex layout and cannot warm a future shader.
 * Explicit draw ranges still permit geometry generated by a vertexNode. */
function hasDrawableGeometry(mesh: WarmupMesh): boolean {
  const geometry = mesh.geometry;
  const count = geometry.index?.count ?? geometry.getAttribute("position")?.count ??
    (Number.isFinite(geometry.drawRange.count) ? geometry.drawRange.count : 0);
  return count > 0 && geometry.drawRange.count !== 0;
}
...
export async function warmScenePaced(
  ...
  for (const mesh of meshes) {
    if (!hasDrawableGeometry(mesh)) continue;
  ...
  for (const mesh of representatives) {
    ...
    // A pooled mesh can also park its geometry while the dedup sweep yields.
    if (!attached || !hasDrawableGeometry(mesh)) continue;
...
export async function warmRootPaced(
  ...
  // Hide every mesh, including duplicates. Hiding only representatives left
  // hundreds of duplicates in EVERY batch, defeating signature deduplication.
  const hidden = meshes.map((mesh) => ({
    mesh,
    visible: mesh.visible,
    frustumCulled: mesh.frustumCulled
  }));
  for (const entry of hidden) entry.mesh.visible = false;
  const representativeEntries = representatives.map(mesh => hidden.find(entry => entry.mesh === mesh)!);
  ...
  let batchSize = 1;
  try {
    for (let index = 0; index < representativeEntries.length; ) {
      const batch = representativeEntries.slice(index, index + batchSize);
      index += batch.length;
      ...
      for (const entry of batch) {
        let parent = entry.mesh.parent;
        while (parent && parent !== root.parent) {
          if (!parent.visible) { ancestors.add(parent); parent.visible = true; }
      ...
      try {
        ...
        for (const ancestor of ancestors) ancestor.visible = false;
      }
