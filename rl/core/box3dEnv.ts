 ...
 */
import { BodyType } from "../../src/core/box3dWorld.ts";
import {
  advancePhase,
  decode,
  observe,
...
// Disturbance multiplier for the recovery-training shove (Node-only env; see step()).
const SHOVE_SCALE = Number(process.env.SHOVE ?? 1) || 1;
...
export class Box3DEnv {
  ...
  step(action: ArrayLike<number>): StepResult {
    ...
    if (this.stepCount > 50 && this.stepCount % 100 === 0) {
      ...
      // SHOVE env knob scales disturbance for the "balance chaos" experiment
      // (goat trains at SHOVE=2: twice the kick, learns hard recovery).
      const kick = this.torsoMass * (0.4 + this.rng() * 0.7) * SHOVE_SCALE;
