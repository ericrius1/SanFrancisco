...
async function main() {
  ...
  const proc = spawn(chrome, [
    ...
    "--hide-scrollbars", "--mute-audio", `--window-size=${W},${H}`, `${SERVER_URL}/?autostart=1&profile=1&fullfps=1`
  ...
  const t0 = Date.now();
  let ready = false;
  while (Date.now() - t0 < 150000) { try { if (await ev(c, `!!(window.__sf&&window.__sf.garden&&window.__sf.wildlands&&window.__sf.player)`)) { ready = true; break; } } catch {} await sleep(600); }
  ...
  const readyStats = await ev(c, `(async()=>{
    const sf=window.__sf;
    await Promise.all([sf.garden.ready,sf.wildlands.ready]);
    return {garden:sf.garden.stats,wildlands:sf.wildlands.stats};
  })()`);
  ...
  for (const view of VIEWS) {
    ...
    try {
      ...
      const diag = await ev(c, `(()=>{const w=window.__sf.wildlands;const read=(s)=>({nearActive:s.nearActive(),instances:s.instances,chunks:s.chunks,designs:s.designs});return{main:read(w.trees.stats),buenaVista:read(w.buenaVistaTrees.stats)};})()`);
