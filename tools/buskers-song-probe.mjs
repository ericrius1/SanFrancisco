// Structural audit of the busker trio's authored score (src/gameplay/buskers/song.ts).
...
import { SONG, SONG_BEATS, BEATS_PER_BAR, SEC_PER_BEAT } from "../src/gameplay/buskers/song.ts";
...
for (const [part, events] of Object.entries(SONG)) {
  ...
  for (const e of events) {
    ...
    prev = e.beat;
    if (e.beat < 0 || e.beat >= SONG_BEATS) fail(`${part}: onset ${e.beat} outside song`);
    if (e.beat + e.dur > SONG_BEATS + 1e-6) fail(`${part}: event at ${e.beat} rings past song end`);
...
const COLOUR = { Dm: [4, 0], Bb: [7, 0, 9], F: [2, 7], C: [9, 2] }; // allowed non-triad pcs (Bb gets the maj7)
for (const e of SONG.flute) {
  if (e.dur < 1) continue;
  const bar = Math.floor(e.beat / BEATS_PER_BAR) + 1;
...
const counts = Object.fromEntries(Object.entries(SONG).map(([k, v]) => [k, v.length]));
console.log(`song: ${SONG_BEATS} beats (${(SONG_BEATS * SEC_PER_BEAT).toFixed(1)}s), events:`, counts);
if (failures) {
  console.error(`${failures} failure(s)`);
  process.exit(1);
}
console.log("score audit clean — all parts diatonic, sorted, in range, chord-true");
