...
const OUT = path.join(ROOT, ".data", "probe-gg");
...
async function main() {
  ...
  try {
    ...
    // free-camera shots: freeze chase, drive camera directly
    const CAMS = [
      ["v3_day_join", -2925, 85, -2750, -2967, 66, -2820, 14.0],
      ["v3_day_rail_above", -2940, 95, -3550, -2965, 62, -3200, 14.0],
      ["v3_night_join", -2925, 85, -2750, -2967, 66, -2820, 21.5]
    ];
    for (const [name, x, y, z, tx, ty, tz, tod] of CAMS) {
      await waitEval(c, "Boolean(window.__sf && window.__sf.player)", 120000);
      await evaluate(c, `(() => {
        const s = window.__sf;
        s.sky.cycleEnabled = false;
        s.sky.setTimeOfDay(${tod});
      ...
      await sleep(1600);
      await evaluate(c, `(() => { const s = window.__sf; s.camera.position.set(${x},${y},${z}); s.camera.lookAt(${tx},${ty},${tz}); return 'ok'; })()`);
      await sleep(200);
      const shot = await c.send("Page.captureScreenshot", { format: "jpeg", quality: 90 });
      const f = path.join(OUT, `cand_${name}.jpg`);
      writeFileSync(f, Buffer.from(shot.data, "base64"));
      console.log(`[probe] ${name} -> ${path.relative(ROOT, f)}`);
    }
    c.close();
  } finally {
    chrome.kill("SIGTERM");
