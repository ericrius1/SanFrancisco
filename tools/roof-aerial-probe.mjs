...
async function main() {
  ...
  try {
    ...
    for (const s of SPOTS) {
      ...
      await cam(0); await tick(c); await shot(c, `roofa_${s.name}_f1.png`);
      await cam(0); await tick(c); await shot(c, `roofa_${s.name}_f2.png`);
      await cam(0.03); await tick(c); await shot(c, `roofa_${s.name}_dolly.png`);
    }
