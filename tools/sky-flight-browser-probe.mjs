...
try {
 ...
 const takeoffInput=await page.evaluate(()=>{
  ...
  sf.physics.world.setBodyVelocity(sf.player.body,[0,0,0],[0,0,0]);
  let holdFrame=0;
  sf.input.setDriver({update(dt,c){if(holdFrame++===0)c.tap('Space');c.hold('Space')}});
  ...
  sf.player.skyFlight.reset(sf.player);
  sf.physics.world.setBodyVelocity(sf.player.body,[0,0,0],[0,0,0]);
  sf.input.setDriver({update(dt,c){c.hold('KeyW');c.hold('ShiftLeft')}});
  for(let i=0;i<90;i++)sf.tick(1/60);
  const shiftRun={active:sf.player.skyFlight.active,speed:sf.player.speed,rise:sf.player.velocity.y};
  sf.input.setDriver(null);
  window.__sfManual(false);
  return {tap,beforeHold,afterHold,shiftRun};
 });
 ...
 check('Shift run never starts flight',!takeoffInput.shiftRun.active&&takeoffInput.shiftRun.speed>0,takeoffInput.shiftRun);
