// Drives tools/bundle-depth-probe.html headless and asserts sprite occlusion
...
let chrome;
try {
  await waitHttp(`http://127.0.0.1:${vitePort}/tools/bundle-depth-probe.html`, 30000);
  chrome = spawn(
  ...
  await send("Page.navigate", { url: `http://127.0.0.1:${vitePort}/tools/bundle-depth-probe.html` });
  const t = Date.now();
  let result;
