...
async function main() {
  ...
  const r = await ev(c, `(async()=>{
    ...
    let nodes = 0, meshes = 0, autoM = 0, invisible = 0;
    sf.scene.traverse(o => { nodes++; if (o.isMesh||o.isSprite) meshes++; if (o.matrixAutoUpdate) autoM++; if (!o.visible) invisible++; });
    const time = (fn, n) => { const a = performance.now(); for (let i=0;i<n;i++) fn(); return +((performance.now()-a)/n).toFixed(3); };
    // matrix pass alone (second call = no-op dirty flags, so force via matrixWorldNeedsUpdate not possible cheaply — measure first call after a tick)
    sf.tick(1/60); await dev.queue.onSubmittedWorkDone();
    const mw = time(() => sf.scene.updateMatrixWorld(true), 20); // force=true every node
    const mwLazy = time(() => sf.scene.updateMatrixWorld(false), 20);
    // full tick cpu
    const cpu = [];
    for (let i=0;i<40;i++){ const a=performance.now(); sf.tick(1/60); const b=performance.now(); await dev.queue.onSubmittedWorkDone(); cpu.push(b-a); }
    cpu.sort((a,b)=>a-b);
    return { nodes, meshes, autoM, invisible, mwForce: mw, mwLazy, tickCpuP50: +cpu[cpu.length>>1].toFixed(2) };
  })()`);
