  ...
  for (const stop of STOPS) {
    ...
    const census = await ev(c, `(()=>{
      ...
      sf.scene.traverse((o) => {
        ...
        const g = groups[key] ?? (groups[key] = { draws: 0, tris: 0, meshes: 0, instanced: 0, shadow: 0 });
        const t = trisOf(o);
        g.draws += 1; g.tris += t; g.meshes += 1; if (o.isInstancedMesh) g.instanced += 1;
        if (o.castShadow) g.shadow += 1;
        if (key === "Group" || key === "Mesh") {
          let label = "", p2 = o;
          while (p2 && p2 !== sf.scene) { if (p2.name) { label = p2.name; break; } p2 = p2.parent; }
          if (!label) label = (o.material && o.material.name) || o.type;
          const mk = label + " :: " + (o.name || o.type);
          mystery[mk] = (mystery[mk] || 0) + 1;
        }
      ...
      return { groups, heavy: heavy.slice(0, 25), mystery };
    })()`);
    ...
    console.log("  --- Group/Mesh mystery breakdown ---");
    for (const [k, n] of Object.entries(census.mystery || {}).sort((a, b) => b[1] - a[1]).slice(0, 40)) {
      console.log(`  ${String(n).padStart(4)}x  ${k.slice(0, 80)}`);
    }
  }
